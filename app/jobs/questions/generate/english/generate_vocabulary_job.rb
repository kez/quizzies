# Questions::Generate::English::GenerateVocabularyJob.perform_now
class Questions::Generate::English::GenerateVocabularyJob < ApplicationJob
  def perform(args = nil)
    ...
    @words.each do |word|
      puts "Running #{word}"
      next unless Rails.cache.read("english:vocabulary:#{word}")
      data_string = Rails.cache.fetch("english:vocabulary:#{word}") do
