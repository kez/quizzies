class Quiz::QuestionController < ApplicationController
  ...
  def update
    ...
    redirect_to quiz_question_path(quiz.to_param, quiz_question.ordinal), flash: {danger: "That's not quite correct, try again or skip if you're stuck!"} and return unless answer_is_correct

    quiz_question.update(answered: 1)

    if quiz_question.ordinal == @questions.count - 1
      ahoy.track "Quiz Completed", {quid_id: quiz.id}
      if quiz.finished_at.nil?
        quiz.finished_at = DateTime.now
        quiz.save
        flash[:confetti] = true
      end
